...
See all suppliers at a glance, with a clear indication of which are active and which are suspended.
Filter by country (Colombia / USA) to see only suppliers relevant to each market.
Filter by category to answer questions like "what active meat suppliers do we have in the USA?".
Register a new supplier from a form.
Update an existing supplier's rate from the interface.
Activate or suspend a supplier with a single click.
