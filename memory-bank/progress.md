...
- Scaffolded Next.js backoffice at `uis/backoffice` (port 3002): auth guard + `/login`, inventory pages under `/backoffice/inventory/*` (ingredients with stock bands, inbound/outbound forms, order history), `src/lib/inventory.ts` API client; prior static HTML archived under `legacy/`.
